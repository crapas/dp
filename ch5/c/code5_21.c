...
int LPS_length(char *str, int n)
{
  ...
  // 최대 회문 부분 시퀀스의 길이를 저장합니다.
  int LPSL_table[n][n];

  // 길이가 1인 문자열은 그 자체로 회문이므로 LPSL의 값은 1 입니다.
  for(int i = 0; i < n; i++)
    LPSL_table[i][i] = 1;
  
  // 길이가 2인 문자열의 LPSL부터 차례로 채워 나갑니다.
  for(int k = 2; k <= n; k++)
  {
    for(int i = 0; i < n - k + 1; i++)
    {
      int j = i + k - 1;
      // 길이가 2이며 두 글자가 모두 같으면 LPSL은 2 입니다.
      if(str[i] == str[j] && k == 2)
        LPSL_table[i][j] = 2;
      // 앞 뒤 글자가 같으면 두 글자를 제외한 문자열의 
      // LPS에 두 글자를 추가합니다. (LPSL_table[i + 1][j - 1] + 2)
      else if(str[i] == str[j])
        LPSL_table[i][j] = LPSL_table[i + 1][j - 1] + 2;
      // 앞 뒤 글자가 다른 경우 LPS가 바뀌지 않으므로 
